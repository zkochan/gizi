...
function GitServer(repoPath, options) {
  ...
  if (!options.source) {
    throw new Error('git server requires options.source');
  }
  ...
  entry.getBlob()
    .then(function(blob) {
      ...
      if (this._entriesCount === 0) {
        ...
        executeCommands(this._cmds, function() {
          publish(this._source, this._destination, function(err) {
            if (err) {
